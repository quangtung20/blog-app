...
const blogCtrl = {
  createBlog: async (req: IAuth, res: Response) => {
  ...
  updateBlog:async(req:IAuth,res:Response)=>{
    if(!req.user) 
  ...
  deleteBlog:async(req:IAuth,res:Response)=>{
    try {
      if(!req.user){
        return res.status(400).json({msg: "Invalid Authentication."})
      }
      ...
      if(!blog){
        return res.status(400).json({msg: "This blog does not exist"})
      }
